...
for(game in games) {
    ...
    const gameStrings = games[game].split(':')[1].split(' ');
    for(i in gameStrings) {
        const elem = gameStrings[i];
        if(!isNumber(elem) && elem !== '') {
            const num = gameStrings[i-1];
            if(elem.includes('red') && num > +colorCounts[0]){
                colorCounts[0] = num;
                console.log(colorCounts[0]);
            }
            if(elem.includes('green') && num > +colorCounts[1])
                colorCounts[1] = num;
            if(elem.includes('blue') && num > +colorCounts[2])
                colorCounts[2] = num;
        }
    }
    console.log('========')
    const minPower = colorCounts[0] * colorCounts[1] * colorCounts[2];
    sum += minPower;
}

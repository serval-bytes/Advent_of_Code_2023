    ...
    for(let j = 0; j < schematicLines[i].length; j++) {
        if(schematicLines[i][j] === '*') {
            ...
            if(adjCells.num === 2) {
                let tempQuotient = 1;
                for(let k = 0; k < 2; k++) {
                    let cols=adjCells.numArray[k][1];
                    let startingColRight = cols + 1;

                    let lookAhead = schematicLines[adjCells.numArray[k][0]][cols];
                    const lookAheadCopy = lookAhead;
                    
                    let leftSideTemp = '';
                    let rightSideTemp = '';
                    let leftSideComputed = false;
                    let rightSideComputed = false;

                    if(isNumber(schematicLines[adjCells.numArray[k][0]][cols-1])) {
                        leftSideComputed = true;
                        while(isNumber(lookAhead)) {
                            leftSideTemp = lookAhead + '' + leftSideTemp;
                            cols--;
                            lookAhead = schematicLines[adjCells.numArray[k][0]][cols];
                        }
                    }
                    if(isNumber(schematicLines[adjCells.numArray[k][0]][startingColRight])) {
                        rightSideComputed = true;
                        lookAhead = (leftSideComputed ? '': lookAheadCopy) + schematicLines[adjCells.numArray[k][0]][startingColRight];
                        while(isNumber(lookAhead)) {
                            rightSideTemp = rightSideTemp + '' + lookAhead;
                            startingColRight++;
                            lookAhead = schematicLines[adjCells.numArray[k][0]][startingColRight];
                        }
                    }
                    if(!leftSideComputed && !rightSideComputed) 
                        leftSideTemp = lookAhead;
                    tempQuotient *= +(leftSideTemp + rightSideTemp);
                }
                sum+=tempQuotient;
            }
        }

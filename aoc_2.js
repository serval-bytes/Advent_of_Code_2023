...
const colorCount_red = 12;
const colorCount_green = 13;
const colorCount_blue = 14;
const isNumber = (val) => { return !isNaN(+val) && isFinite(+val); }
let sum = 0;
const countOfColors = {
    "red": colorCount_red,
    "green": colorCount_green,
    "blue": colorCount_blue
}


for(game in games) {
    const splitGames = games[game].split(':');
    const gameString = splitGames[1];
    const gameId = splitGames[0].split(' ')[1];
    const colorCount = gameString.split(' ');
    gameString[0] = gameId;
    let count = 0;
    let color = '';
    let addToSum = true;

    for(index in colorCount) {
        
        elem = colorCount[index];
        if(isNumber(elem)) {
            count = elem;
        }
        else if(elem !== '') {
            color = elem.replace(/[^a-zA-Z0-9]/g, "");
        }
        if(count !== 0 && color !== '') {
            if(count > countOfColors[color]) 
                addToSum = false;
            count = 0;
            color = '';
        }
    }
    if(addToSum) { sum += +(gameId); addToSum = true; }
}

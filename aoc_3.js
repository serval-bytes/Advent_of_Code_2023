input = `416.........................559...............417...............785.......900.......284...........503...796....992..........................
...
for(i in schematicLines) {
    const line = schematicLines[i];
    const prevNums = nums;
    const prevSymbols = symbols;
    let prevNumsFound = false;
    let prevSymsFound = false;
    nums = {};
    symbols = [];
    if(prevNums != {})
        prevNumsFound = true;
   ...
   for(symbol in symbols) {
    for(num in nums) {
        if(nums[num] !== -1 && +symbols[symbol] >= +num - 1 && +symbols[symbol] <= +num + nums[num].length)
        {
            sum += +(nums[num]);
            nums[num] = -1;
        }
    ...
    if(prevNumsFound) {
        ...
        for(num in prevNums) {
            if(prevNums[num] !== -1 && +symbols[symbol] >= num - 1 && +symbols[symbol] <= +num + prevNums[num].length)
            {
                console.log(prevNums[num]);
                sum += +(prevNums[num]);
                prevNums[num] = -1;
            }
        }
    ...
    if(prevSymsFound) {
        for(symbol in prevSymbols) {
            for(num in nums) {
                if(nums[num] !== -1 && +prevSymbols[symbol] >= +num - 1 && +prevSymbols[symbol] <= +num + nums[num].length)
                {
                    sum += +(nums[num]);
                    nums[num] = -1;
                }
